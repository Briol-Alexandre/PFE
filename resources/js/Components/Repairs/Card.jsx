...
export default function RepairCard({ repairs }) {
    return (
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 px-10">
            {repairs.map((repair) => (
                <li key={repair.id} className="bg-black/20 backdrop-blur-3xl lg:max-w-[400px] aspect-square p-10 rounded-3xl hover:bg-white/20 transition-colors duration-300">
                    <Link href={route('repair.show', { repair: repair.id })} className="flex flex-col gap-4">
                        ...
                        <div className='flex flex-col'>
                            ...
                            <div className="mt-4 space-y-1">
                                <p className="text-sm text-gray-400">Révision : {repair.revisions.map(revision => revision.name).join(', ')}</p>
